#!/usr/bin/env bash
# lowakernel-universalbuild script
# builds for all amd thinkpads, zen1 and newer
# just run it, should work. tested on p14s gen6
# if something breaks open an issue i guess

set -euo pipefail

KERNEL_VERSION="${KERNEL_VERSION:-}"
KERNEL_NAME="lowakernel-universal"
JOBS=$(nproc)
...
# grab the right zen tag and checkout
fetch_source() {
...
# the big one - strip all the crap we dont need
apply_config() {
...
build_kernel() {
  ...
  # x86-64-v3 works on zen1+ so t495 and everything after
  export KCFLAGS="-O2 -pipe -march=x86-64-v3 -mtune=generic"
  ...
  make ARCH=x86_64 \
       ...
       KCPPFLAGS="$KCPPFLAGS" \
       -j"$JOBS" \
       2>&1 | tee "$BUILD_DIR/build-universal.log"
...
# install everything and make the uki
install_kernel() {
  local kver
  ...
  local fw_dropin="/etc/mkinitcpio.conf.d/lowakernel-amdgpu-fw.conf"
...
main() {
  echo -e "\n${BOLD}${CYAN}╔════════════════════════════════════════════════════╗"
  echo -e "║  lowakernel-universal — AMD ThinkPad Kernel Builder  ║"
  echo -e "║  Target: All AMD ThinkPads (Zen1+), x86-64-v3       ║"
  echo -e "║  T495 · T14/T14s/T16 G1-G6 · P14s G1-G6            ║"
  echo -e "║  P15v G1-G3 · L14/L15 G1-G4                        ║"
  echo -e "╚════════════════════════════════════════════════════╝${NC}\n"
  ...
  success "lowakernel-universal installed!"
  echo -e "  UKI:  /boot/EFI/Linux/${KERNEL_NAME}.efi"
  echo -e "  Log:  $BUILD_DIR/build-universal.log"
  echo -e "  ${CYAN}Reboot and select lowakernel-universal${NC}"
  echo -e "${BOLD}${GREEN}════════════════════════════════════════${NC}"
}
